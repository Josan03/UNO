...
export interface IndexedGame extends UnoGame.Game {
    readonly id: string
...
export class ServerModel {
    ...
    async add(creator: string, numberOfPlayers: number) {
        const g = await this.gameStore.add_pending({ creator, numberOfPlayers, players: [], pending: true })
    ...
    private startGameIfReady(pending_game: PendingGame): Promise<ServerResponse<IndexedGame | PendingGame, StoreError>> {
        ...
        if (pending_game.players.length === pending_game.numberOfPlayers) {
            const game = UnoGame.createUnoGame(pending_game.players, 500, { randomizer: this.randomizer, startRound: true })
    ...
    private async update(id: string, player: string, processor: (game: IndexedGame) => Promise<unknown>)
        : Promise<ServerResponse<IndexedGame, ServerError>> {
        let uno: ServerResponse<IndexedGame, ServerError> = await this.game(id)
        uno = await uno.filter(async game => {
