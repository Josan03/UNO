export type Resolvers<Value, Error, U> = {
  onSuccess: (value: Value) => Promise<U>;
  onError: (error: Error) => Promise<U>;
};
...
export interface ServerResponse<Value, Error> {
  ...
  flatMap<T, E>(
    f: (v: Value) => Promise<ServerResponse<T, E>>
  ): Promise<ServerResponse<T, Error | E>>;
  filter<E>(
    p: (v: Value) => Promise<boolean>,
    error: (v: Value) => Promise<E>
  ): Promise<ServerResponse<Value, Error | E>>;
}
...
class ErrorResponse<Value, Error> implements ServerResponse<Value, Error> {
  ...
  async filter<E>(
    _p: (v: Value) => Promise<boolean>,
    _error: (v: Value) => Promise<E>
  ) {
    return this;
  }
}
