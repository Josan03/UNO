...
export function MongoStore(connectionString: string, dbName: string, randomizer: Randomizer): GameStore {
    async function from_mongo(m: WithId<IndexedMemento>): Promise<IndexedGame> {
        return { ...from_memento(m, randomizer), id: m._id.toHexString() }
    }

    async function to_mongo(y: IndexedGame): Promise<WithId<IndexedMemento>> {
    ...
    const uno = (db: Db) => db.collection<IndexedMemento>('uno')
    const pending = (db: Db) => db.collection<Omit<PendingGame, "id">>('uno.pending')

    return {
        games(): Promise<ServerResponse<IndexedGame[], StoreError>> {
            return dbQuery(async db => {
                const all = await uno(db).find().toArray()
                return await Promise.all(all.map(from_mongo))
            })
        },
        game(id: string): Promise<ServerResponse<IndexedGame, StoreError>> {
            return dbQuery(async db => {
                const game = await uno(db).findOne({ _id: ObjectId.createFromHexString(id) })
                if (!game) return game
        ...
        add(game: IndexedGame): Promise<ServerResponse<IndexedGame, StoreError>> {
            return dbQuery(async db => {
                await uno(db).insertOne(await to_mongo(game), { ignoreUndefined: true })
                return game
            })
        },
        update(game: IndexedGame): Promise<ServerResponse<IndexedGame, StoreError>> {
            return dbQuery(async db => {
                const result = await uno(db).replaceOne({ _id: ObjectId.createFromHexString(game.id) }, await to_mongo(game))
