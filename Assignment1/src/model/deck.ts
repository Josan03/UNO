...
export const types: Readonly<Type[]> = ['NUMBERED', 'SKIP', 'REVERSE', 'DRAW', 'WILD', 'WILD DRAW'] as const
...
export class ArrayDeck implements Deck {
    ...
    fromMemento(cards: Record<string, string | number>[]): Deck {
        ...
        for (const card of cards) {
            ...
            if (card.type === 'NUMBERED' && ((typeof card.color !== 'string' || !colors.includes(card.color as Color)) || (typeof card.number !== 'number' || card.number < 0 || card.number > 9))) {
                throw new Error(`Invalid NUMBERED card: ${JSON.stringify(card)}`);
            }
            if (card.type === 'SKIP' && (typeof card.color !== 'string' || !colors.includes(card.color as Color))) {
                throw new Error(`Invalid SKIP card: ${JSON.stringify(card)}`);
            }
            if (card.type === 'REVERSE' && (typeof card.color !== 'string' || !colors.includes(card.color as Color))) {
                throw new Error(`Invalid REVERSE card: ${JSON.stringify(card)}`);
            }
            if (card.type === 'DRAW' && (typeof card.color !== 'string' || !colors.includes(card.color as Color))) {
...
export function createFromMemento(cards: Record<string, string | number>[]): Deck {
