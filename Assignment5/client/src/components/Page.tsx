...
export default function Page({ children }: { children: React.ReactNode }) {
  ...
  const pending_games = useSelector<State, PendingGamesState>(
    (state) => state.pending_games
  );

  const is_participant = (g: { players: readonly string[] }) =>
    g.players.some(_.isEqual(player));

  const my_ongoing_games = ongoing_games.filter(is_participant);
  ...
  return (
    <>
      ...
      <nav>
        ...
        <h2>My Games</h2>
        <h3>Ongoing</h3>
        {my_ongoing_games.length === 0 ? (
          <p>No ongoing games</p>
        ) : (
          my_ongoing_games.map((game) => (
            <Link className="link" to={`/game/${game.id}`} key={game.id}>
              Game #{game.id}
            </Link>
          ))
        )}
        ...
        {my_pending_games.length === 0 ? (
          <p>No pending games</p>
        ) : (
          my_pending_games.map((game) => (
            <Link className="link" to={`/pending/${game.id}`} key={game.id}>
              Game #{game.id} ({game.players.length}/{game.numberOfPlayers})
            </Link>
          ))
        )}

        <h2>Available Games</h2>
        {other_pending_games.length === 0 ? (
          <p>No available games</p>
        ) : (
          other_pending_games.map((game) => (
            <Link className="link" to={`/pending/${game.id}`} key={game.id}>
              Game #{game.id} ({game.players.length}/{game.numberOfPlayers})
            </Link>
          ))
